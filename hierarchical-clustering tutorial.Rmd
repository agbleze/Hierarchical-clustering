...
Now the fuel_expenditure data has been transformed, we need to make sure that the second data is also transform to be in the same quality level or format. Remember that health_expenditure is for 6 months which means that in order to comparing data of the same time scale, we need to convert it to monthly data and then calculate the mean. 

The code code to transform health expenditure is below;     
...
Now, we have transformed both datasets and we need verify if they are equal in terms of number of rows and columns before joining together for further analysis.

```{r}
nrow(fuel_expenditure_transformed)
nrow(health_expenditure_transformed)
```
We realize that the number of rows are different for fuel_expenditure-transformed (37) and health_expenditure_transformed (36). 
This is a common occurrence in data exploration and preparation. There is a state without data in health_expenditure_transformed. If we are to inspect the data, we will realize that there is no record for  Jigawa state. This is an instance of where we are faced with the decision of either employing a data imputation method to provide values for Jigasaw or leaving it out altogether. We will tackle data imputation in a different post. For now, the better choice will be to leave Jigawa state at of the analysis. So we will remove the row for Jigawa state in fuel_expenditure_transformed in order to have 36 rows for all datasets and then join them together as well as convert the column **state_names** to row names as this is what will be used to label the cluster leafs to be more meaningful. The code for that is as follows;


```{r}
# fuel_expenditure_transformed <- fuel_expenditure_transformed[!(fuel_expenditure_transformed$state == 17)] #%>%
#   #dplyr::filter(fuel_expenditure_transformed, fuel_expenditure_transformed$state_names != "Jigawa") 
...
datatable(dataset_all)
 ...
 # expenditure_all <- left_join(fuel_expenditure_transformed, health_expenditure_transformed, by = state_names)
 # datatable(expenditure_all)
```

Now our data is already and prepared for analysis. But before then, we have to do clustering relevant data preparation

#### Clustering relevant data preparation
The data preparation done at this stage is not one that we will do for all analysis. We are doing them in order to successfully do a cluster analysis that is appropriate.
...
* Computation of dissimilarity matrix: This is done identify observations that can belong to the same clusters. 
...
Agglomerative clustering can be undertaken using hclust(), flashClust or agnes()
...
Using agnes function gives us the agglomerative co-efficent (ac) which is measures the level of clustering. The higher the ac, the stronger the clustering structure.
...
Now, let all the methods we have discuss for AGNES.
...
#agnes_centriod <- agnes(dataset_dissimilarity, method = "median")
...
From the various methods, agnes with the ward produces an ac of 0.9 which is the highest compared to others hence produces the strongest structure for the dataset.


Before we draw the curtains down on agnes, let also note the hcluster() fucntion also accepts other methods for clustering such as the following
...
```{r}
try.diss.div <- diana(dataset_dissimilarity)
pltree(try.diss.div, cex = 0.6, hang = .2)
try.diss.div$dc
try.diss.div$order.lab

```

#### Identifying clusters of subgroups
We can identify observations that falls into similar groups or otherwise by stating the number of groups we want to cut into. Lets cuts our observation into 4 clusters with the code below. This can be done for both agnes and divise
...
### Comparing dendrograms
 We can compare the various clustering for different methods by creating dendrogram objects for them and
 plotting to compare
...
```
The dashed lines shows cluster combinations different from those in the other tree. 

#### customizing the dendrogram
Visualizing results has never become less important for cluster analysis where observations are group and there is the need for more colourful differentiation of groups to better understand the visualization. There are options to for this.
...
We have added entanglement co-efficient to the graph. It measures how well align the trees on a scale of 1 to 0. The low co-oefficient of 0.17 shows least entanglement with good alignment

### Determining optimal clusters
A number of methods can be used to determine the optimal of clusters for grouping
...
```
To determine optimal cluster with elbow method, we need to look at where a bend is located which can be gauged to 3 from the graph above.

### Average silhouette method
Average silhouette methods indicates the how well the clusters form with within cluster difference minimized at the optimal cluster number. Thus, the higher the average cluster with the better the clustering for different number of clusters.

```{r}
fviz_nbclust(dataset_all_scaled, FUN = hcut, method = "silhouette")
```
From the plot, the optimal number of clusters specified by average silhouette method is 3.
...
The gaps method identified 1 to be the optimal number of clusters within a range of 1 to 10 clusters. Well, as we can deduce, different methods can identify different number of clusters. With 2 of the methods suggesting 3 to be the optimal number of clusters, we can go by that. 

Now that we know the optimal cluster number, let use that to visualize the data but in a different way from the previous
...
plot(as.phylo(hcl_complete), type = "fan", tip.color = colors[ward_cut], label.offset = .5, cex = 0.7, no.margin = T, use.edge.length = T)
```

## Conclusion
This tutorial demonstrate hierarchical clustering us a project that is typical of the real world. We went through process of data wrangling and transformation, clustering and determining the optimal clustering.
